...
df <- read.csv("stats-v0.52.0-2.csv", stringsAsFactors=FALSE)
...
```{r}
r_df <- df %>%
    filter(parameter_mode != "loading" & parameter_node == 20) %>%
...
ggplot(r_df) +
    geom_path(aes(x = size, y = mean, color = parameter_lang, group = parameter_lang)) +
    geom_point(aes(x = size, y = mean, color = parameter_lang), size=4) +
    labs(x = "xxx", y = "Time (s)", title = "Loading -- size=n, ncalls=0") +
    theme_minimal()
```
...
```{r}
ipred_df <- df %>%
    dplyr::filter(!grepl("data-00MB", command)) %>%
